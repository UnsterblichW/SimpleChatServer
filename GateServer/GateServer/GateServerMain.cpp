...
void testredispp() {
    try {
        ...
        connection_options.host = "192.168.23.128";
        connection_options.password = "123456";
        ...
        std::unordered_map<std::string, double> scores = {{"m2", 2.3},
                                                          {"m3", 4.5}};
        ...
        redis.zrangebyscore("sorted_set",
                            BoundedInterval<double>(
                                1.5, 3.4, BoundType::LEFT_OPEN), // (1.5, 3.4]
...
int main()
{
    try {
        ...
        unsigned short port = static_cast<unsigned short>(gate_port_str.empty() ? 8080 : std::stoi(gate_port_str));
