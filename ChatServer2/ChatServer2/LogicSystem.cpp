...
void LogicSystem::DealMsg()
{
    for (;;) {
        ...
        if (call_back_iter == _fun_callbacks.end()) {
            _msg_que.pop();
            std::cout << "msg id [" << msg_node->_recvnode->_msg_id << "] handler not found"
                      << std::endl;
            continue;
        }
        call_back_iter->second(msg_node->_session,
...
void LogicSystem::RegisterCallBacks()
{
    ...
    _fun_callbacks[ID_AUTH_FRIEND_REQ] = std::bind(&LogicSystem::AuthFriendApply,
                                                   this,
                                                   std::placeholders::_1,
                                                   std::placeholders::_2,
                                                   std::placeholders::_3);

    _fun_callbacks[ID_TEXT_CHAT_MSG_REQ] = std::bind(&LogicSystem::DealChatTextMsg,
                                                     this,
                                                     std::placeholders::_1,
                                                     std::placeholders::_2,
                                                     std::placeholders::_3);
                               ...
                               const std::string& msg_data)
{
    ...
    Json::Value rtvalue;
    DeferFuncWapper defer([this, &rtvalue, session]() {
                                 ...
                                 const std::string& msg_data)
{
    ...
    std::cout << "user login uid is  " << uid << " applyname  is " << applyname << " bakname is "
              << bakname << " touid is " << touid << std::endl;
    ...
    // 如果想要添加的好友正好和申请人处在同一个服务器，直接通知对方有申请消息
