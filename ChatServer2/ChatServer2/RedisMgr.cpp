...
bool RedisMgr::LPush(std::string const &key, std::string const &value) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
    }
...
bool RedisMgr::LPop(std::string const &key, std::string &value) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
    }
...
bool RedisMgr::RPush(std::string const &key, std::string const &value) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
    }
...
bool RedisMgr::RPop(std::string const &key, std::string &value) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
    }
...
bool RedisMgr::HSet(std::string const &key, std::string const &hkey,
...
bool RedisMgr::HSet(char const *key, char const *hkey, char const *hvalue,
                    size_t hvaluelen) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
...
std::string RedisMgr::HGet(std::string const &key, std::string const &hkey) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return "";
    }
...
bool RedisMgr::HDel(std::string const &key, std::string const &field) {
    auto connect = _con_pool->getConnection();
    if (connect == nullptr) {
        return false;
    }

    DeferFuncWapper defer([&connect, this]() { _con_pool->returnConnection(connect); });
